...
(async () => {
    let feed = await parser.parseString(fs.readFileSync(`webdb/${process.env.DATEMARK}/solidot/solidot.xml`));

    const outputLatex = feed.items.map(item => {
        return sanitizeTextForLatex(`\\entryitemGeneric{\\hskip 0pt{}${item.title}}{${item.link.replace(/\#.+$/, '')}}`);
    }).join('\n\n');

    fs.writeFileSync(`webdb/${process.env.DATEMARK}/final/solidot.tex`, outputLatex);
})();
